...
bool mmodbus_sendRaw(uint8_t *data, uint16_t size, uint32_t timeout)
{
 ...
 // mmodbus_delay(1);
  ...
  LL_DMA_EnableIT_TE(_MMODBUS_DMA, _MMODBUS_DMASTREAM);
  LL_USART_ClearFlag_TC(_MMODBUS_USART);
...
//##################################################################################################
bool mmodbus_readCoil(uint8_t slaveAddress, uint16_t number, uint16_t *data)
{
  return mmodbus_readCoils(slaveAddress, number, 16, data);
}
//##################################################################################################
bool mmodbus_readCoils(uint8_t slaveAddress, uint16_t startnumber, uint16_t length, uint16_t *data)
{
  #if( _MMODBUS_RTU == 1)
  uint8_t txData[8];
...
bool mmodbus_readHoldingRegisters8i(uint8_t slaveAddress, uint16_t startnumber, uint16_t length, uint8_t *data)
{
  #if( _MMODBUS_RTU == 1)
  uint8_t txData[8];
  txData[0] = slaveAddress;  // Địa chỉ thiết bị slave
  txData[1] = MModbusCMD_ReadHoldingRegisters; // Chức năng đọc Holding Registers
  txData[2] = (startnumber & 0xFF00) >> 8;     // Byte cao của địa chỉ bắt đầu
  txData[3] = (startnumber & 0x00FF);			// Byte thấp của địa chỉ bắt đầu
  txData[4] = (length & 0xFF00) >> 8; 			// Byte cao của độ dài
  txData[5] = (length & 0x00FF);				// Byte thấp của độ dài
  static uint16_t  crc;
  crc = mmodbus_crc16(txData, 6);			// Tính toán CRC
  txData[6] = (crc & 0x00FF);
  txData[7] = (crc & 0xFF00) >> 8;
  mmodbus_sendRaw(txData, 8, 100);				// Gửi dữ liệu
  uint16_t recLen = mmodbus_receiveRaw(mmodbus.timeout);	// Nhận dữ liệu phản hồi
  if(recLen == 0)
    return false; // Không nhận được dữ liệu
  if(mmodbus.rxBuf[0] != slaveAddress)
    return false;	// Địa chỉ slave không khớp
  if(mmodbus.rxBuf[1] != MModbusCMD_ReadHoldingRegisters)
    return false;	// Chức năng không khớp
  crc = mmodbus_crc16(mmodbus.rxBuf, mmodbus.rxBuf[2] + 3);	// Tính toán CRC
  if(((crc & 0x00FF) != mmodbus.rxBuf[mmodbus.rxBuf[2] + 3]) || (((crc & 0xFF00) >> 8) != mmodbus.rxBuf[mmodbus.rxBuf[2] + 4]))
  ...
  if(data != NULL)
  {
    ...
    {    
      ...
      mmodbus.rxBuf[i+3+1] = H;      	// Hoán đổi byte cao và thấp
    }
    memcpy(data, &mmodbus.rxBuf[3], mmodbus.rxBuf[2]);   	// Sao chép dữ liệu vào buffer
  }
  return true;
  #endif
}
...
{
  return mmodbus_readHoldingRegisters16i(slaveAddress, number, 2, data);
}
//##################################################################################################
